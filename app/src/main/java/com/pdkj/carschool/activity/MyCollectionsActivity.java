...
import com.pdkj.carschool.databinding.ActivityMyCollectionsBinding;
...
public class MyCollectionsActivity extends BaseActivity<ActivityMyCollectionsBinding> implements
        ...
        public void run() {
            ...
            if (type == 1){
                cursor = sqlBrite.rawQueryDb("select * from " + QuestionsMetaData.MetaData.TABLE_NAME_COLLECTIONS_SUBJECT1
                        + " where " + QuestionsMetaData.MetaData.EXAMTYPEID + " = ?", new String[]{SharedPreferenceUtils.getStringValue(mContext,"examTypeId")});
            }else if (type == 2){
    ...
    private void insertError(QuestionsBean bean, String myAnswer) {
        ...
        if (type == 1){
            sqlBrite.insertErrorSubject1(QuestionsBean.getContentValues(bean, timeDate, myAnswer,SharedPreferenceUtils.getParams("examTypeId",mContext),""));
        }else if (type == 2){
            sqlBrite.insertErrorSubject4(QuestionsBean.getContentValues(bean, timeDate, myAnswer,SharedPreferenceUtils.getParams("examTypeId",mContext),""));
        }
    }
