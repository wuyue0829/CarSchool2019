...
import com.pdkj.carschool.databinding.ActivityMyErrorBinding;
...
public class MyErrorSubjectActivity extends BaseActivity<ActivityMyErrorBinding> implements
    ...
    private Runnable runnable = new Runnable() {
        ...
        public void run() {
            ...
            Cursor cursor = null;
            if (type == 1){
                cursor = sqlBrite.rawQueryDb("select * from " + QuestionsMetaData.MetaData.TABLE_NAME_ERROR_SUBJECT1
                        + " where " + QuestionsMetaData.MetaData.EXAMTYPEID + " = ?" + " order by " + QuestionsMetaData.MetaData.TIME + " DESC ", new String[]{SharedPreferenceUtils.getStringValue(mContext,"examTypeId")});
            }else if (type == 2){
                cursor = sqlBrite.rawQueryDb("select * from " + QuestionsMetaData.MetaData.TABLE_NAME_ERROR_SUBJECT1
                        + " where " + QuestionsMetaData.MetaData.EXAMTYPEID + " = ?" + " order by " + QuestionsMetaData.MetaData.TIME + " DESC ", new String[]{SharedPreferenceUtils.getStringValue(mContext,"examTypeId")});
            }
